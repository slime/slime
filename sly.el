...
(defvar
  sly-cheat-sheet-table
  '((:title
     "Editing lisp code"
     :map sly-mode-map
     :bindings ((sly-eval-defun "Evaluate current top level form")
                (sly-compile-defun "Compile current top level form")
                (sly-interactive-eval "Prompt for form and eval it")
                (sly-compile-and-load-file "Compile and load current file")
                (sly-sync-package-and-default-directory
                 "Synch default package and directory with current buffer")
                (sly-next-note "Next compiler note")
                (sly-previous-note "Previous compiler note")
                (sly-remove-notes "Remove notes")
                sly-documentation-lookup))
    (:title "Completion"
            :map sly-mode-map
            :bindings (sly-indent-and-complete-symbol
    ...
    (:title
     ...
     :bindings ((sly-inspector-next-inspectable-object
                 "Jump to next inspectable object")
                (sly-inspector-operate-on-point
                 "Inspect object or execute action at point")
                (sly-inspector-reinspect "Reinspect current object")
                (sly-inspector-pop "Return to previous object")
                ;;(sly-inspector-copy-down "Send object at point to REPL")
                (sly-inspector-toggle-verbose "Toggle verbose mode")
                (sly-inspector-quit "Quit")))
    (:title
     "Finding Definitions"
     :map sly-mode-map
     :bindings (sly-edit-definition
                sly-pop-find-definition-stack))))

(defun sly-cheat-sheet ()
  (interactive)
  (switch-to-buffer-other-frame
   (get-buffer-create (sly-buffer-name :cheat-sheet)))
  (setq buffer-read-only nil)
  (delete-region (point-min) (point-max))
  (goto-char (point-min))
  (insert
   "SLY: The Superior Lisp Interaction Mode for Emacs (minor-mode).\n\n")
  (dolist (mode sly-cheat-sheet-table)
