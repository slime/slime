...
(defun %eval-region (string)
  (with-input-from-string (stream string)
    (let (- values)
	 ...
	 (setq - form)
	 (if *sly-repl-eval-hooks* 
	     (setq values (run-repl-eval-hooks form))
	     (setq values (multiple-value-list (eval form))))
	 (finish-output))))))

(defun run-repl-eval-hooks (form)
  (loop for hook in *sly-repl-eval-hooks* 
	for res =  (catch *sly-repl-eval-hook-pass* 
		     (multiple-value-list (funcall hook form)))
	until (not (eq res *sly-repl-eval-hook-pass*))
	finally (return 
		  (if (eq res *sly-repl-eval-hook-pass*)
...
(defun %listener-eval (string)
  ...
  (with-buffer-syntax ()
    (track-package 
     (lambda ()
       (let ((*sly-repl-suppress-output* :unset)
	     (*sly-repl-advance-history* :unset))
	 (multiple-value-bind (values last-form) (%eval-region string)
	   (unless (or (and (eq values nil) (eq last-form nil))
		       (eq *sly-repl-advance-history* nil))
	     (setq *** **  ** *  * (car values)
		   /// //  // /  / values))
	   (setq +++ ++  ++ +  + last-form)
	   (unless (eq *sly-repl-suppress-output* t)
