...
(def-sly-test complete-symbol*
  ...
  (let ((completions (sly-completions prefix)))
    (sly-test-expect "Completion set" expected-completions completions)))

(def-sly-test complete-form
  ...
  (with-temp-buffer
    ...
    (sly-complete-form)
    (sly-check-completed-form buffer-sexpr wished-completion)
    ...
    (unless skip-trailing-test-p
      ...
      (sly-complete-form)
      (sly-check-completed-form (concat buffer-sexpr ")") wished-completion))
    ))

(defun sly-check-completed-form (buffer-sexpr wished-completion)
  (sly-test-expect (format "Completed form for `%s' is as expected"
                              buffer-sexpr)
                     wished-completion
                     (buffer-string)
                     'equal))

(provide 'sly-c-p-c-tests)
