...
(define-sly-contrib sly-editing-commands
  ...
  (:license "GPL")
  (:on-load
   (define-key sly-mode-map "\M-\C-a"  'sly-beginning-of-defun)
   (define-key sly-mode-map "\M-\C-e"  'sly-end-of-defun)
   (define-key sly-mode-map "\C-c\M-q" 'sly-reindent-defun)
   (define-key sly-mode-map "\C-c\C-]" 'sly-close-all-parens-in-sexp)))

(defun sly-beginning-of-defun ()
  (interactive)
  (if (and (boundp 'sly-repl-input-start-mark)
           sly-repl-input-start-mark)
      (sly-repl-beginning-of-defun)
    (let ((this-command 'beginning-of-defun)) ; needed for push-mark
      (call-interactively 'beginning-of-defun))))

(defun sly-end-of-defun ()
  (interactive)
  (if (eq major-mode 'sly-repl-mode)
      (sly-repl-end-of-defun)
      (end-of-defun)))

(defvar sly-comment-start-regexp
  "\\(\\(^\\|[^\n\\\\]\\)\\([\\\\][\\\\]\\)*\\);+[ \t]*"
  "Regexp to match the start of a comment.")

(defun sly-beginning-of-comment ()
...
Otherwise leave point unchanged and return NIL."
  (let ((boundary (point)))
    (beginning-of-line)
    (cond ((re-search-forward sly-comment-start-regexp boundary t)
           (point))
          (t (goto-char boundary) 
             nil))))

(defvar sly-close-parens-limit nil
  "Maxmimum parens for `sly-close-all-sexp' to insert. NIL
means to insert as many parentheses as necessary to correctly
close the form.")

(defun sly-close-all-parens-in-sexp (&optional region)
  ...
  (let ((sexp-level 0)
    ...
    (when (> sexp-level 0)
      ...
      (let* ((deleted-region     (delete-and-extract-region point (point)))
        ...
        (when sly-close-parens-limit
          (let ((missing-parens (- sexp-level prior-parens-count
                                   sly-close-parens-limit)))
            (dotimes (i (max 0 missing-parens))
              (delete-char -1))))))))

(defun sly-insert-balanced-comments (arg)
...
(defun sly-remove-balanced-comments ()
...
;; SLY-CLOSE-PARENS-AT-POINT is obsolete:

;; It doesn't work correctly on the REPL, because there
;; BEGINNING-OF-DEFUN-FUNCTION and END-OF-DEFUN-FUNCTION is bound to
;; SLY-REPL-MODE-BEGINNING-OF-DEFUN (and
;; SLY-REPL-MODE-END-OF-DEFUN respectively) which compromises the
;; way how they're expect to work (i.e. END-OF-DEFUN does not signal
;; an UNBOUND-PARENTHESES error.)

;; Use SLY-CLOSE-ALL-PARENS-IN-SEXP instead.

;; (defun sly-close-parens-at-point ()
;;   "Close parenthesis at point to complete the top-level-form.  Simply
;; inserts ')' characters at point until `beginning-of-defun' and
;; `end-of-defun' execute without errors, or `sly-close-parens-limit'
;; is exceeded."
;;   (interactive)
;;   (loop for i from 1 to sly-close-parens-limit
;;         until (save-excursion
;;                 (sly-beginning-of-defun)
;;                 (ignore-errors (sly-end-of-defun) t))
;;         do (insert ")")))

(defun sly-reindent-defun (&optional force-text-fill)
...
be treated as a paragraph.  This is useful for filling docstrings."
  (interactive "P")
  (save-excursion
    (if (or force-text-fill (sly-beginning-of-comment))
        (fill-paragraph nil)
      (let ((start (progn (unless (or (and (zerop (current-column))
                                           (eq ?\( (char-after)))
                                      (and sly-repl-input-start-mark
                                           (sly-repl-at-prompt-start-p)))
                            (sly-beginning-of-defun))
                          (point)))
            (end (ignore-errors (sly-end-of-defun) (point))))
        (unless end
          (forward-paragraph)
